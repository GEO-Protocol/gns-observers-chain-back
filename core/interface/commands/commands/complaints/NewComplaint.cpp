#include "NewComplaint.h"

NewComplaintCommand::NewComplaintCommand(
        ...
        identifier())
{
    ...
    if (commandBuffer.size() < minCommandLength) {
        throw ValueError(
                "NewComplaintCommand: can't parse command. "
...
const string &NewComplaintCommand::identifier()
{
    static const string identifier = "GET:stats/balance/total";
    return identifier;
}

const vector<NodeUUID>& NewComplaintCommand::gateways() const
{
    return mGateways;
}

const SerializedEquivalent NewComplaintCommand::equivalent() const
{
    return mEquivalent;
}

CommandResult::SharedConst NewComplaintCommand::resultOk(
